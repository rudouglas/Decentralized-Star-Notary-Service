...
// Creates a random id up to max value
const createRandomTokenId = (max=1000) => {
...
it("lets user2 buy a star and decreases its balance in ether", async () => {
  ...
  await instance.buyStar(starId, { from: user2, value: balance, gasPrice: 106469 });
...
it("can add the star name and star symbol properly", async () => {
  ...
  let tokenId = createRandomTokenId();
  let instance = await StarNotary.deployed();
  await instance.createStar("Test Star", tokenId, {from: accounts[0]});

  //2. Call the name and symbol properties in your Smart Contract and compare with the name and symbol provided
  assert.equal(await instance.name.call(), "Chaos Star Token");
  assert.equal(await instance.symbol.call(), "KOS");
});

it("lets 2 users exchange stars", async () => {
    let instance = await StarNotary.deployed();
    let user1 = accounts[1];
    let tokenId1 = createRandomTokenId();
    let user2 = accounts[2];
    let tokenId2 = createRandomTokenId();
